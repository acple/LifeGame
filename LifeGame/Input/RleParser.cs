...
public class RleParser : IParser
{
    ...
    private static readonly Parser<char, ImmutableHashSet<Cell>> parser = CreateParser();

    private static Parser<char, ImmutableHashSet<Cell>> CreateParser()
    {
        ...
        Parser<char, State> DeadCell(int count, State state)
            => deadCell.MapConst(state with { X = state.X + count });

        Parser<char, State> AliveCell(int count, State state)
            => aliveCell.MapConst(state with { X = state.X + count, Cells = state.Cells.Union(Enumerable.Range(state.X, count).Select(x => new Cell(x, state.Y))) });

        Parser<char, State> Newline(int count, State state)
            => newline.MapConst(state with { X = 0, Y = state.Y + count });

        var rle = Pure(new State(0, 0, []))
            .Chain(state => end.MapConst(state) | count.Bind(count => Choice(DeadCell(count, state), AliveCell(count, state), Newline(count, state))));
